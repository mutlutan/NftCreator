	...
	/*Abonelik Durumları*/
	CREATE TABLE dbo.TemAbonelikDurum(
		Id			INT NOT NULL,
		...
		CONSTRAINT PK_TemAbonelikDurum PRIMARY KEY (Id)
	);
	CREATE UNIQUE INDEX UX_TemAbonelikDurum_Ad ON TemAbonelikDurum (Ad);
	CREATE INDEX IX_TemAbonelikDurum_Durum ON TemAbonelikDurum (Durum);
	INSERT INTO TemAbonelikDurum (Id, Durum, Sira, Kod, Ad, Aciklama) VALUES (0, 1, -9, N'', N'', N'');
	INSERT INTO TemAbonelikDurum (Id, Durum, Sira, Kod, Ad, Aciklama) VALUES (1, 1, 1, N'ACTIVE', N'Aktif', N'Bir abonelik aktif ise ve ödemeler düzenli olarak alınıyorsa status active olur');
	INSERT INTO TemAbonelikDurum (Id, Durum, Sira, Kod, Ad, Aciklama) VALUES (2, 1, 2, N'PENDING', N'Durduruldu', N'Bir abonelik durdurulmuşsa status pending olur');
	INSERT INTO TemAbonelikDurum (Id, Durum, Sira, Kod, Ad, Aciklama) VALUES (3, 1, 3, N'UNPAID', N'Ödeme Alınamadı', N'Abonelik sırasında ödeme alınamamış ise status unpaid olur');
	INSERT INTO TemAbonelikDurum (Id, Durum, Sira, Kod, Ad, Aciklama) VALUES (4, 1, 4, N'UPGRADED', N'Başka Plana Geçmiş', N'Abonelik başka bir plan ile güncellenmişse status upgraded olur');
	INSERT INTO TemAbonelikDurum (Id, Durum, Sira, Kod, Ad, Aciklama) VALUES (5, 1, 5, N'CANCELED', N'İptal Edilmiş', N'Abonelik iptal edilmişse status cancelled olur');
	INSERT INTO TemAbonelikDurum (Id, Durum, Sira, Kod, Ad, Aciklama) VALUES (6, 1, 6, N'EXPIRED', N'Abonelik Dönemi Bitmiş', N'Abonelik periyodu bitmişse status expired olur. Karttan başka bir ödeme alınmaz');

	/*Abonelik Urun*/
	CREATE SEQUENCE dbo.sqTemAbonelikUrun AS INT START WITH 101 INCREMENT BY 1;
	CREATE TABLE dbo.TemAbonelikUrun(
		Id			INT NOT NULL,

		Durum		BIT NOT NULL,  
		Sira		INT NOT NULL,
		Kod		    NVARCHAR(50), /*Referans kodu*/
		Ad		    NVARCHAR(50)
		
		CONSTRAINT PK_TemAbonelikUrun PRIMARY KEY (Id)
	);
	CREATE UNIQUE INDEX UX_TemAbonelikUrun_Ad ON TemAbonelikUrun (Ad);
	CREATE INDEX IX_TemAbonelikUrun_Durum ON TemAbonelikUrun (Durum);
	INSERT INTO TemAbonelikUrun (Id, Durum, Sira, Kod, Ad) VALUES (0, 1, -9, N'', N'');
	INSERT INTO TemAbonelikUrun (Id, Durum, Sira, Kod, Ad) VALUES (Next Value For dbo.sqTemAbonelikUrun, 1, 1, N'f5e6b618-0dcd-44db-81f2-cf26cc23dbb2', N'Eğitim Platformu Aboneliği');

	/*Abonelik Plan Ödeme Donem*/
	CREATE TABLE dbo.TemAbonelikDonem(
		Id			INT NOT NULL,
		...
		CONSTRAINT PK_TemAbonelikDonem PRIMARY KEY (Id)
	);
	CREATE UNIQUE INDEX UX_TemAbonelikDonem_Ad ON TemAbonelikDonem (Ad);
	CREATE INDEX IX_TemAbonelikDonem_Durum ON TemAbonelikDonem (Durum);
	INSERT INTO TemAbonelikDonem (Id, Durum, Sira, Ad) VALUES (0, 1, -9, N'');
	INSERT INTO TemAbonelikDonem (Id, Durum, Sira, Ad) VALUES (11, 1, 1, N'Günlük');
	INSERT INTO TemAbonelikDonem (Id, Durum, Sira, Ad) VALUES (12, 1, 2, N'Haftalık');
	INSERT INTO TemAbonelikDonem (Id, Durum, Sira, Ad) VALUES (13, 1, 3, N'Aylık');
	INSERT INTO TemAbonelikDonem (Id, Durum, Sira, Ad) VALUES (14, 1, 4, N'Yıllık');

	/*Abonelik Urun*/
	CREATE SEQUENCE dbo.sqTemAbonelikUrunPlan AS INT START WITH 1 INCREMENT BY 1;
	CREATE TABLE dbo.TemAbonelikUrunPlan(
		Id					INT NOT NULL,
		AbonelikUrunId		INT NOT NULL,
		...
		AbonelikDonemId		INT NOT NULL,
		Ucret				DECIMAL(18,2) NOT NULL,
		ParaBirimId			INT NOT NULL,

		Aciklama			NVARCHAR(max),
		
		CONSTRAINT PK_TemAbonelikUrunPlan PRIMARY KEY (Id),
		CONSTRAINT FK_TemAbonelikUrun_AbonelikUrunId FOREIGN KEY (AbonelikUrunId) REFERENCES TemAbonelikUrun (Id),
		CONSTRAINT FK_TemAbonelikUrun_AbonelikDonemId FOREIGN KEY (AbonelikDonemId) REFERENCES TemAbonelikDonem (Id),
		CONSTRAINT FK_TemAbonelikUrun_ParaBirimId FOREIGN KEY (ParaBirimId) REFERENCES TemParaBirim(Id)
	);
	CREATE INDEX IX_TemAbonelikUrunPlan_AbonelikUrunId ON TemAbonelikUrunPlan (AbonelikUrunId);
	CREATE INDEX IX_TemAbonelikUrunPlan_Durum ON TemAbonelikUrunPlan (Durum);
	INSERT INTO TemAbonelikUrunPlan (Id, AbonelikUrunId, AbonelikDonemId, Durum, Sira, Kod, Ad, Ucret, ParaBirimId) VALUES (0, 0, 0, 0, -9, N'', N'', 0, 0);
	INSERT INTO TemAbonelikUrunPlan (Id, AbonelikUrunId, AbonelikDonemId, Durum, Sira, Kod, Ad, Ucret, ParaBirimId) VALUES (Next Value For dbo.sqTemAbonelikUrunPlan, 101, 13, 1, 1, N'ec5d2fc1-3ef3-4fd0-8eba-d52df9fca079', N'Aylık Abonelik', 35, 1);
	INSERT INTO TemAbonelikUrunPlan (Id, AbonelikUrunId, AbonelikDonemId, Durum, Sira, Kod, Ad, Ucret, ParaBirimId) VALUES (Next Value For dbo.sqTemAbonelikUrunPlan, 101, 14, 1, 2, N'62bfeb55-6e94-49e1-9bf9-e4705bc3cc1a', N'Yıllık Abonelik', 336, 1);

	/*KullaniciAbonelik - iyzico abonelik işlenleri*/
	CREATE SEQUENCE dbo.sqTemKullaniciAbonelik AS INT START WITH 1 INCREMENT BY 1;
	CREATE TABLE dbo.TemKullaniciAbonelik(
		Id					INT NOT NULL,
		KullaniciId			INT NOT NULL,  /*iyzico servisinde conversationId için kullanılır*/
		AbonelikDurumId		INT NOT NULL,

		AbonelikUrunPlanId	INT NOT NULL,
		 
		Jeton				NVARCHAR(50),
		OdemeFormuBilgileri	NVARCHAR(max),
		...
		CONSTRAINT PK_TemKullaniciAbonelik PRIMARY KEY (Id),
		CONSTRAINT FK_TemKullaniciAbonelik_KullaniciId FOREIGN KEY (KullaniciId) REFERENCES TemKullanici (Id),
		CONSTRAINT FK_TemKullaniciAbonelik_AbonelikDurumId FOREIGN KEY (AbonelikDurumId) REFERENCES TemAbonelikDurum (Id),
		CONSTRAINT FK_TemKullaniciAbonelik_AbonelikUrunPlanId FOREIGN KEY (AbonelikUrunPlanId) REFERENCES TemAbonelikUrunPlan (Id)
	);
	CREATE INDEX IX_TemKullaniciAbonelik_KullaniciId ON TemKullaniciAbonelik (KullaniciId);
	CREATE INDEX IX_TemKullaniciAbonelik_AbonelikDurumId ON TemKullaniciAbonelik (AbonelikDurumId);
	CREATE INDEX IX_TemKullaniciAbonelik_AbonelikUrunPlanId ON TemKullaniciAbonelik (AbonelikUrunPlanId);

	/*KullaniciAbonelikOdeme - iyzico abonelik işlenleri*/
	CREATE SEQUENCE dbo.sqTemKullaniciAbonelikOdeme AS INT START WITH 1 INCREMENT BY 1;
	CREATE TABLE dbo.TemKullaniciAbonelikOdeme(
		Id						INT NOT NULL,
		KullaniciAbonelikId		INT NOT NULL,  /*iyzico servisinde conversationId için kullanılır*/

		Durum					BIT NOT NULL,
		BaslamaTarihi			DATE,		/*Lisans başlama*/
		BitisTarihi				DATE,		/*Lisans bitiş - bitiş tarihi geçen demo olur*/

		YanitIcerigi			NVARCHAR(max),
		...
		CONSTRAINT PK_TemKullaniciAbonelikOdeme PRIMARY KEY (Id),
		CONSTRAINT FK_TemKullaniciAbonelikOdeme_KullaniciAbonelikId FOREIGN KEY (KullaniciAbonelikId) REFERENCES TemKullaniciAbonelik (Id)
	);
	CREATE INDEX IX_TemKullaniciAbonelikOdeme_KullaniciAbonelikId ON TemKullaniciAbonelikOdeme (KullaniciAbonelikId);
