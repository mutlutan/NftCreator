window.mnYetkiSablon = function () {
    ...
    function getAuthority() {
        for (var i = 0; i < mnApp.areas.length; i++) {
            var area_name = mnApp.areas[i];
            $.ajax({
                url: "/Areas/" + area_name + "/Authority/" + area_name + ".js?" + mnApp.version,
                dataType: 'script', async: false,
                success: function (result, textStatus, jqXHR) {
                    var yetki = eval(area_name + "Authority");
